...
function drawRegions(geo){
  regionLayer = L.geoJSON(geo,{
    ...
    onEachFeature:(f,l)=>{
      ...
      l.on('mouseover',()=>l.setStyle({color:'#f1c40f',weight:3}).bringToFront());
      l.on('mouseout', ()=>l.setStyle({color:'#444',weight:1}));
    }
  }).addTo(map);
}
...
function spawn(user,reg,color){
  ...
  const m=L.circleMarker([lat,lon],{
      radius:4,fillColor:color,fillOpacity:.9,color:'#000',weight:.4
  }).bindTooltip(user);
  m.addTo(map);
...
function animateMove(user,reg){
  ...
  let i=0;
  const k=setInterval(()=>{
    if(++i>=steps){ obj.marker.setLatLng([lat,lon]);clearInterval(k);}
...
function popup(feat,layer){
  ...
  layer.bindPopup(
    `<div class="infocard">
       <h3>${niceName(feat)}</h3>
       <p>ID : ${id}<br>Contrôle : <b>${camp}</b><br>Combattants : <b>${fighters}</b></p>
     </div>`,
...
socket.on('init', ({regions,game})=>{
  ...
  drawRegions(regions);
  /* on ajoute les pions APRES que la carte existe */
  queueMicrotask(()=>{
    Object.entries(game.viewers).forEach(([u,v])=>spawn(u,v.region,campColors[v.camp]));
  });
});
